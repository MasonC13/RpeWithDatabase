    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if existing_athlete:
                ...
                existing_athlete.last_4_digits = str(row.get('Last 4 Digits', ''))
                existing_athlete.last_name = row.get('Last Name', '')
                existing_athlete.first_name = row.get('First Name', '')
                existing_athlete.position = row.get('Position', '')
                existing_athlete.summer_attendance = row.get('Summer Attendance', '')
                ...
                athlete = Athlete(
                    email=email,
                    last_4_digits=str(row.get('Last 4 Digits', '')),
                    last_name=row.get('Last Name', ''),
                    first_name=row.get('First Name', ''),
                    position=row.get('Position', ''),
                    summer_attendance=row.get('Summer Attendance', '')
                )
                session.add(athlete)
                athletes_migrated += 1
